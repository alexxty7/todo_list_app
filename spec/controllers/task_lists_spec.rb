    ...
    context "authenticated user" do
      ...
      %w(id title).each do |attr|
        it "task_list object contains #{attr}" do
          expect(response.body).to be_json_eql(task_list.send(attr.to_sym).to_json).at_path("0/#{attr}")
        end
      end

      context 'tasks' do
        it 'included in task_list object' do
          expect(response.body).to have_json_size(1).at_path('0/tasks')
        end
        
        %w(id description completed position deadline).each do |attr|
          it "contains #{attr}" do
            expect(response.body).to be_json_eql(task.send(attr.to_sym).to_json).at_path("0/tasks/0/#{attr}")
          end
        end
      end
    end
